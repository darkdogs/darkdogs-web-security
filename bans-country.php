...
if (isset($_GET['blacklist'])) {
    ...
    $query = mysqli_query($connect, "UPDATE `$table` SET countryban_blacklist='Yes' WHERE id=1");
}

if (isset($_GET['whitelist'])) {
    $table = $prefix . 'settings';
    $query = mysqli_query($connect, "UPDATE `$table` SET countryban_blacklist='No' WHERE id=1");
}
?>
...
if (isset($_POST['ban-country'])) {
    ...
    if ($redirect == 'Yes' and $url == NULL) {
        ...
        $queryvalid = mysqli_query($connect, "SELECT * FROM `$table` WHERE country='$country' LIMIT 1");
        $validator  = mysqli_num_rows($queryvalid);
        if ($validator > "0") {
            echo '<br />
		<div class="callout callout-info">
                <p><i class="fa fa-info-circle"></i> This <strong>Country</strong> is already banned.</p>
        </div>
		';
        } else {
            $query = mysqli_query($connect, "INSERT INTO `$table` (country, redirect, url) VALUES('$country', '$redirect', '$url')");
        }
    }
}
?>
...
if (isset($_GET['edit-id'])) {
    ...
    if (isset($_POST['edit-ban'])) {
		...
		';
            ...
            $update = mysqli_query($connect, "UPDATE `$table` SET country='$country', redirect='$redirect', url='$url' WHERE id='$id'");
        }
    }
    ...
    if (empty($id)) {
        echo '<meta http-equiv="refresh" content="0; url=bans-country">';
        exit();
    }
 ...
 <form class="form-horizontal" action="" method="post">
                    <div class="box">
						<div class="box-header">
							<h3 class="box-title">Edit - Country Ban #<?php
    echo $id;
?></h3>
						</div>
						<div class="box-body">
...
if ($row['countryban_blacklist'] == "No") {
    echo 'btn-success';
} else {
    echo 'btn-default';
}
...
<form class="form-horizontal" action="" method="post">
				     <div class="box">
						<div class="box-header">
							<h3 class="box-title">Ban Country</h3>
						</div>
				        <div class="box-body">
										<div class="form-group">
...
<option value="Zimbabwe">Zimbabwe</option>
	...
	<select name="redirect" class="form-control" required>
        ...
        <option value="Yes">Yes</option>
                        ...
                        <div class="panel-footer">
							<button class="btn btn-flat btn-danger" name="ban-country" type="submit">Ban</button>
							<button type="reset" class="btn btn-flat btn-default">Reset</button>
				        </div>
				     </div>
</form>
